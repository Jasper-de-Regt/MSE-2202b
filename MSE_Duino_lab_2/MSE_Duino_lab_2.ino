...
// to change number of leds, just update the first 2 lines of code
...
const unsigned int potentiometerPin = 14;                         // assigns an analog pin to the potentiometer. Note that pins above 13 are analong pins. So potentiometerPin-14=Apin. The dial type pot is pin P4 on J2
...
void loop() {
  ...
  // going to plan on receiving serial every 0.5 second. If no serial has been received for 1 seconds, go ahead and read buttons and switches and stuff
  // kind of like a watchdog, if (serial.avail()) resets the watchdog
  if (Serial.available()) {
  ...
  // if the watchdog has expired, use the buttons and switches
    ...
    // if the sliding switch is down, set ledStateChangeDelay based on potentiometer
    ...
    // else if the sliding switch is up, set ledStateChangeDelay based on button press
    else {
      //    bool tempLedStateChangeDelayor = isTheButtonActuallyPressed ();
      isTheButtonActuallyPressed ();
    }
...
// start of functions
// this function will return a bool with a debounced buttonIsPressed state and it will update the ledStateChangeDelay
void isTheButtonActuallyPressed () {
  ...
  if ((millis() - previousButtonStateChangeTime) > 30) {        // the 50 is the debounce time in mS. The button needs to have been in the previous state for this many mS in order to register it as a valid buttonState
    ...
    //    return buttonIsPressed;                                             // return the debounced buttonIsPressed. This is the only way to return a true.
    //break;
  }
